...
| Signature/extract/inline | Advanced | Parameter reorder/add/remove, calls, extraction captures, safe expression/block inline, local/constant introduction, binding conversion, promotion, and function/method conversion are transactionally planned and compiler validated |
| Move | Advanced | One/many declarations and types, split/merge aliases, new-file creation, owned nested identities, attached line comments, header-safe import repair, dependencies, cycles, preview/apply, and compiler validation are tested; generated declarations remain |
| Migration | Advanced | Interface/declaration generation, ownership repairs, compile-time migration, dead-code proof, same-snapshot recipes, and dependent staged recipes with fresh prospective identities are implemented |
